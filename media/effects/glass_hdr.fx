                     ...
                     uniform float4x4 WorldVi) 
{
  ...
  float4 eyePoll = mul (WorldVi, IN.Position);
  OUT.clip0 =  dot(glstate.clip[0].plane,eyePoll);
  OUT.clip1 =  dot(glstate.clip[1].plane,eyePoll);
  return OUT;
}
...
ClipPlaneEnable[0]=true;
    ...
    FrontFace       = CCW;
    CullFaceEnable  = true;
    VertexProgram   = compile latest GlassVS(wvp, world, worldI, viewI,WorldView);
