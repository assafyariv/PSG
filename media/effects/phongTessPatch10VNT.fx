...
)
{
  ...
  // Per patch output
  oEdgeTess[0] = calculateTessFactor( position[3], position[6], position[8], position[9] );
  oEdgeTess[1] = calculateTessFactor( position[0], position[4], position[7], position[9] );
  oEdgeTess[2] = calculateTessFactor( position[0], position[1], position[2], position[3] );
  oEdgeTess[3] = 0.0f;
...
)
{
  ...
  float3 p = bezierTriangle3P( u, v, position );
  float3 n = bezierTriangle3P( u, v, normal );
  float2 t = bezierTriangle2P( u, v, texCoord );
...
technique FragmentLighting_Patches
{
  pass p0
  {
    ...
    TessellationEvaluationProgram = compile gp5tep "TRIANGLES" tessEval( Wvp, World, WorldI, ViewI );
