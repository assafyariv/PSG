...
float4 viewspacePos = mul (worldviewMat,float4(P.xyz,1.0));
float4 viewprojected = mul (gProjection,viewspacePos);
...
float depth =  (-viewspacePos.z - nearp) / (farp-nearp);
//float depth = (-viewprojected.z- nearp) / (farp-nearp);// viewprojected.w;
//depth = farp*nearp/ (depth*(nearp-farp) + farp);
//depth = depth / viewprojected.w;
//depth = depth * 0.5 + 0.5;
//float depth = (-viewspacePos.z / viewspacePos.w) *(farp-nearp);
normColor = float4(N.rg,frac(depth),floor(depth)/256);
...
technique MainMRT
{
	pass p0
	{
		...
		ClipPlaneEnable[0]=true;
		VertexProgram   = compile latest vertex(worldViewProj, world, worldIT, viewI,view);
		FragmentProgram = compile latest fragmentMRT(scene.ambient, material,roughness,minRefl,maxRefl,fresnelEXP,view);
	}
}
