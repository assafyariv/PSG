               ...
               ) 
{
  ...
  float  fres = minRefl + (maxRefl-minRefl) * pow((1.0-abs(dot(viewVector,normal))), fresnelEXP) / (4.0 - 3.0f *(1.0-roughh));
...
//  cubeColor = texCUBE(textureENV, reflVector);
  ...
  float4 diffuseColorTex = float4(tex2D(DiffuseMapSampler, tc));
  float fres = IN.texcoord4.w;

  float4 metalColor = material.ambient*float4(ambientSum,1) + material.ambient*scene.ambient + (float4(diffuseColorTex.xyz,1.0) *  material.diffuse + (1.0-grayTarget) * material.diffuse)*float4(diffuseSum, 1) + float4(specularSum, 1.0)*material.specular;
  ...
  result.w = min(1.0,( (1.0-fres) + material.diffuse.w) * diffuseColorTex.w);
  

//  float4 result = (1.0-fres)*( material.diffuse*float4(diffuseColorTex.xyz,1.0) * (grayTarget) + material.ambient*float4(ambientSum,1)*scene.ambient + material.diffuse*((1-grayTarget)*float4(diffuseSum, 1)) + float4(specularSum, 1.0)*material.specular) + fres*cubeColor;
...
normColor = float4(normal, viewspacePos.z *0.01);
...
technique Main
{
  pass p0 
  {
    ...
    VertexProgram   = compile latest mainVS(Wvp, World, WorldI, ViewI,View, minRefl, maxRefl, fresnelEXP,roughness,diffuseMapPosition);
    FragmentProgram = compile latest mainPS(textureHDR_RGBA,  material, lacquer, Exposure, GrayTarget,roughness,diffuseMapSampler ,reflectivity);
  }
}
...
technique MainMRT
{
  pass p0 
  {
    ...
    VertexProgram   = compile latest mainVS(Wvp, World, WorldI, ViewI,View, minRefl, maxRefl, fresnelEXP,roughness,diffuseMapPosition);
    FragmentProgram = compile latest mainPSMRT(textureHDR_RGBA,  material, lacquer, Exposure, GrayTarget,roughness,diffuseMapSampler ,reflectivity,WorldView);
  }
}