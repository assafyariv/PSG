...
#include <setjmp.h>
#include "../jpeg-8d/JpegLibWin/JpegLibWin/bitmapWrite.h"
...
void j_putRGBScanline(char *jpegline, 
						 int widthPix,
						 char *outBuf,
						 int row);

void j_putGrayScanlineToRGB(char *jpegline, 
						 int widthPix,
						 char *outBuf,
						 int row);
...
char * JpegFile::JpegFileToRGB(std::string fileName,
							   unsigned int *width,
							   unsigned int *height, std::string outFilestr,bool writeFile)

{
	...
	*/
	...
	if (writeFile)
	{
		if ((outfile = fopen(outFilestr.c_str(), "wb")) == NULL) {
	...
	*/ 
	...
	dataBuf=(char *)new char[cinfo.output_width * 3 * cinfo.output_height];
	if (dataBuf==NULL) {
	...
	*/
	while (cinfo.output_scanline < cinfo.output_height) {
		...
		if  (writeFile)
		{
			JDIMENSION num_scanlines = /*(void)*/ jpeg_read_scanlines(&cinfo, dest_mgr->buffer, dest_mgr->buffer_height);
		...
		} else
		{
			...
			if (cinfo.out_color_components==3) {
				...
				j_putRGBScanline((char*)buffer[0], 
									*width,
									dataBuf,
				...
				j_putGrayScanlineToRGB((char*)buffer[0], 
									*width,
									dataBuf,
									cinfo.output_scanline-1);
...
bool JpegFile::GetJPGDimensions(std::string fileName,
								 unsigned int *width,
								 unsigned int *height)
...
char *JpegFile::RGBFromDWORDAligned(char *inBuf,
									 ...
									 unsigned int height)
{
	...
	char *tmp;
	tmp=(char *)new char[height * widthPix * 3];
	if (tmp==NULL)
		return NULL;
...
bool JpegFile::RGBToJpegFile(std::string fileName, 
							char *dataBuf,
							 unsigned int widthPix,
							 unsigned int height,
							bool color, 
							int quality)
{
	...
	char* tmp;
	if (!color) {
		tmp = (char*)new char[widthPix*height];
		if (tmp==NULL) {
...
/* Now use the library's routine to set default compression parameters.
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    ...
    (void) jpeg_write_scanlines(&cinfo, (JSAMPARRAY)&outRow, 1);
  }
...
bool JpegFile::RGBToBitmapFile(std::string fileName, 
							char *dataBuf,
							 unsigned int widthPix,
							 unsigned int height,
							bool color, 
							int quality)
{
	// still doesnt work, how to read buffer direftly to bitmap file ?
#if 0
	if (dataBuf==NULL)
		return FALSE;
	if (widthPix==0)
...
char * JpegFile::MakeDwordAlignedBuf(char *dataBuf,
									  ...
									  unsigned int *uiOutWidthBytes)		// bytes!!!
{
	...
	long dwNewsize=(long)((long)uiWidthBytes * 
							(long)height);
	char *pNew;

	////////////////////////////////////////////////////////////
	// alloc and open our new buffer
	pNew=(char *)new char[dwNewsize];
	if (pNew==NULL) {
		return NULL;
	}
	...
	////////////////////////////////////////////////////////////
	// copy row-by-row
	 unsigned int uiInWidthBytes = widthPix * 3;
...
bool JpegFile::VertFlipBuf(char  * inbuf, 
					    unsigned int widthBytes, 
					    unsigned int height)
{   
	char  *tb1;
	...
	tb1= (char *)new char[bufsize];
	if (tb1==NULL) {
		return FALSE;
	}

	tb2= (char *)new char [bufsize];
	if (tb2==NULL) {
		delete [] tb1;
		return FALSE;
	}
...
bool JpegFile::BGRFromRGB(char *buf,  unsigned int widthPix,  unsigned int height)
{
	if (buf==NULL)
		return FALSE;
...
bool JpegFile::MakeGrayScale(char *buf,  unsigned int widthPix,  unsigned int height)
{
	if (buf==NULL)
		return FALSE;
