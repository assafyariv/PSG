...
namespace nvmath
{
  ...
  template<typename T> class Quatt
  {
    public:
      ...
      explicit Quatt( const Matnnt<3,T> & rot );
   ...
   *  equation holds:
   *  \code
   *    Matnnt<3,T>( q0 * q1 ) == Matnnt<3,T>( q0 ) * Matnnt<3,T>( q1 )
   *  \endcode */
  template<typename T>
   ...
   *  q1. */
  NVSG_API Quatt<float> lerp( float alpha, const Quatt<float> & q0, const Quatt<float> & q1 );
   ...
   *  q1. */
  NVSG_API void lerp( float alpha, const Quatt<float> & q0, const Quatt<float> & q1, Quatt<float> &qr );
  ...
  inline Quatt<T>::Quatt( const Vecnt<4,T> & v )
  {
    ...
    m_quat[2] = v[2];
    m_quat[3] = v[3];
    NVSG_PRIVATE_ASSERT( isNormalized( *this ) );
  }
  ...
  {
    NVSG_PRIVATE_ASSERT( isNormalized( v0 ) && isNormalized( v1 ) && ( v0 != - v1 ) );
  ...
  template<typename T>
  inline Quatt<T>::Quatt( const Matnnt<3,T> & rot )
  {
    NVSG_PRIVATE_ASSERT( isRotation( rot ) );
  ...
  inline Quatt<T> & Quatt<T>::normalize()
  {
    ...
    NVSG_PRIVATE_ASSERT( std::numeric_limits<T>::epsilon() < mag );
    T invMag = T(1) / mag;
    for ( int i=0 ; i<4 ; i++ )
    {
      m_quat[i] = m_quat[i] * invMag;
    }
    //  NVSG_PRIVATE_ASSERT( isNormalized( *this ) ); // due to rounding errors this assertion might fail !
    return( *this );
  }
  ...
  {
    return(   ( abs( q0[0] - q1[0] ) < std::numeric_limits<T>::epsilon() )
          &&  ( abs( q0[1] - q1[1] ) < std::numeric_limits<T>::epsilon() )
          &&  ( abs( q0[2] - q1[2] ) < std::numeric_limits<T>::epsilon() )
          &&  ( abs( q0[3] - q1[3] ) < std::numeric_limits<T>::epsilon() ) );
  }
  ...
  {
    NVSG_PRIVATE_ASSERT( isNormalized( q0 ) && isNormalized( q1 ) );
