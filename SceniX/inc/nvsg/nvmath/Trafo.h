...
namespace nvmath
{
  ...
  class Trafo
  {
    public:
       ...
       *  \endcode
       *  \sa getInverse */
      NVSG_API const Mat44f& getMatrix( void ) const;
      ...
      NVSG_API bool operator!=( const Trafo &t ) const;

    private:
      NVSG_API void decompose() const;
  ...
  NVSG_API void lerp( float alpha, const Trafo & t0, const Trafo & t1, Trafo & tr );
  ...
  inline void Trafo::setOrientation( const Quatf &orientation )
  {
    ...
    m_matrixValid = false;
    NVSG_PRIVATE_ASSERT( isNormalized( orientation ) );
  ...
  inline void Trafo::setScaleOrientation( const Quatf &scaleOrientation )
  {
    ...
    m_matrixValid = false;
    NVSG_PRIVATE_ASSERT( isNormalized( scaleOrientation ) );
