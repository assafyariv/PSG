...
namespace nvmath
{
  ...
  {
    return( abs(v0*v1) <= std::max<T>(T(1),length(v0)) * std::max(T(1),length(v1)) * eps );
  }
  ...
  *  \a creaseAngle (in radians), their normals are set to the average of their normals. */
  ...
  {
    return( nvmath::normalize( *this ) );
  }

  template<unsigned int n, typename T>
  template<typename S>
  inline T & Vecnt<n,T>::operator[]( S i )
  {
  ...
  template<unsigned int n, typename T>
  template<typename S>
  inline const T & Vecnt<n,T>::operator[]( S i ) const
  {
    NVSG_CTASSERT( std::numeric_limits<S>::is_integer );
    NVSG_ASSERT( i < n );
    return( m_vec[i] );
  ...
  {
    *this = nvmath::orthonormalize( v, *this );
  }
  ...
  // specializations for Vec3f, Vec4f
  template<>
  inline bool operator<(const Vecnt<3,float>& lhs, const Vecnt<3,float>& rhs)
  {
    return  lhs[0] < rhs[0]  || 
  ...
  template<>
  inline bool operator<(const Vecnt<4,float>& lhs, const Vecnt<4,float>& rhs)
  {
    return  lhs[0] < rhs[0]  || 
            !(lhs[0] > rhs[0]) && lhs[1] < rhs[1] ||
            !(lhs[0] > rhs[0]) && !(lhs[1] > rhs[1]) && lhs[2] < rhs[2] ||
  ...
  inline Vecnt<n,T> orthonormal( const Vecnt<n,T> & v )
  {
    ...
    NVSG_ASSERT( ( T(0) < firstV ) && ( firstI != secondI ) );
