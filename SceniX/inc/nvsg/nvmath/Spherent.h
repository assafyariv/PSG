...
namespace nvmath
{
  ...
  template<unsigned int n, typename T, typename RandomAccessIterator, typename isetType >
    Spherent<n,T> boundingSphere( RandomAccessIterator points, const nvsg::IndexSetReadLock &iset );
#endif
  ...
  template<unsigned int n, typename T>
  inline Spherent<n,T> boundingSphere( const nvmath::Vecnt<n,T> *points, unsigned int numberOfPoints )
  {
    NVSG_ASSERT( points );
...
#if 0
  template<unsigned int n, typename T, typename RandomAccessIterator, typename isetType >
  inline Spherent<n,T> boundingSphere( RandomAccessIterator points, const nvsg::IndexSet * iset )
  {
    nvsg::Buffer::DataReadLock reader( iset->getBuffer() );
    const isetType * indices = reader.getPtr<isetType>();
    unsigned int prIdx = iset->getPrimitiveRestartIndex();

    NVSG_ASSERT(iset->getNumberOfIndices()>1); // requires at least two points
  ...
  Spherent<n,T> boundingSphere( const Spherent<n,T> & s0, const Spherent<n,T> & s1 )
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if ( dist < std::numeric_limits<T>::epsilon() * maxRadius )
        {
