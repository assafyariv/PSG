...
namespace nvutil
{
  ...
  class Trackball
  {
    public:
      //! Default constructor
      NVSG_API Trackball(void);

      //! Default destructor
      NVSG_API virtual ~Trackball(void);
      ...
      /** Calculate the axis and the angle (radians) 
        ...
        * center.
        */
      NVSG_API void  apply( const nvmath::Vec2f &p0   //!< Last mouse position - components must be scaled to [-1,1]
       ...
       */
      NVSG_API void  setSize( float size   //!< %Trackball size
                            );

      //! Determine current size of the trackball.
      /** \return %Trackball size
        */
      NVSG_API float getSize( void );
    ...
    protected:
      ...
      /** Project an x,y pair onto a sphere of radius r OR a hyperbolic sheet
        ...
        */
      NVSG_API float projectOntoTBSphere( const nvmath::Vec2f & p );
