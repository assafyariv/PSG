...
namespace nvutil
{
  ...
  class FileMapping
  {
    ...
    private:
      ...
      struct MapCount
      {
        MapCount() : cnt(0) {} // always default-initialized to 0
        explicit MapCount(int i) : cnt(i) {}
        MapCount& operator++() { ++cnt; return *this; } // preincrement
        MapCount operator++(int) { return MapCount(cnt++); } // postincrement
        MapCount& operator--() { --cnt; return *this; } // predecrement
        MapCount operator--(int) { return MapCount(cnt--); } // postdecrement
        bool operator==(int i) const { return cnt==i; }
        bool operator!=(int i) const { return cnt!=i; }
        operator int() const { return cnt; } // whenever needed as int
        int cnt;
      };
    ...
    private :
      typedef nvutil::STLMap<const void *,MapCount>     OffsetMapCountMap;
      typedef nvutil::STLMap<const void *,ViewHeader *> OffsetPtrViewMap;
      typedef nvutil::STLList<ViewHeader*>              ViewHeaderList;
      ...
      OffsetPtrViewMap  m_offsetPtrViewMap;   // mapping offset pointers to their associated mapped view
      OffsetMapCountMap m_offsetMapCountMap;  // how often is the same offset mapped?
  };
