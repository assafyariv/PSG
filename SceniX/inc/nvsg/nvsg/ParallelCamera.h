...
namespace nvsg
{
   ...
   *  \par Namespace: nvsg
   *  \remarks A ParallelCamera overwrites the projection-specific functions of a FrustumCamera.
   *  \sa Camera, FrustumCamera, PerspectiveCamera */
  class ParallelCamera : public FrustumCamera
  {
    public:
      NVSG_API static ParallelCameraSharedPtr create();

    public:
      /*! \brief Get the projection matrix of this ParallelCamera.
       *  \return The projection transformation.
       *  \sa FrustumCamera, getInverseProjection, getWorldToViewMatrix, getViewToWorldMatrix */
       ...
       *  \sa FrustumCamera, getProjection,  getWorldToViewMatrix, getViewToWorldMatrix */
      ...
      REFLECTION_INFO_API( NVSG_API, ParallelCamera );
    protected:
      friend struct nvutil::Holder<ParallelCamera>;
