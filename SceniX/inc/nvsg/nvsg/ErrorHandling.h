...
// forward declaration
class NVSGException;
/** A SceniX error handler routine is of this type */
typedef void NVSGERRORHANDLER(const NVSGException&);

/** Global pointer to the currently used SceniX error handler routine. Use the 
  * nvsgSetErrorHandler function to assign a new error handler routine .
  */
extern NVSGERRORHANDLER * nvsgErrorHandler;
/** Replace the current SceniX error handler by newErrorHandler 
  *
  * Parameters
  * - newErrorHandler
  *     The replacement for the currently used SceniX error handler routine.
  *
  * Return Value
