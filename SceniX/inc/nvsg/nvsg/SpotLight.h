...
namespace nvsg
{
  ...
  {
    public:
      NVSG_API static SpotLightSharedPtr create();
    ...
    public:
       ...
       *  \return The cut-off angle in radians in the range [0.0, pi/2].
       ...
       *  \param ca The cut-off angle in radians in the range [0.0, pi/2].
       *  \remarks The light's cone is cut off at this angle (in radian). Points beyond this cone
       *  are not lit. The default is pi/4 (or 45°).
       *  \sa getCutoffAngle */
    ...
    protected:
      friend struct nvutil::Holder<SpotLight>;
    ...
    public:
      ...
      REFLECTION_INFO_API( NVSG_API, SpotLight );
      BEGIN_DECLARE_STATIC_PROPERTIES
          NVSG_API DECLARE_STATIC_PROPERTY( Position );
          NVSG_API DECLARE_STATIC_PROPERTY( Attenuation );
          NVSG_API DECLARE_STATIC_PROPERTY( Direction );
          NVSG_API DECLARE_STATIC_PROPERTY( Orientation );
          NVSG_API DECLARE_STATIC_PROPERTY( CutoffAngle );
          NVSG_API DECLARE_STATIC_PROPERTY( FalloffExponent );
  ...
  inline  void  SpotLight::setAttenuation( float c, float l, float q )
  {
    ...
    if ( ( m_attenuation[0] != c ) || ( m_attenuation[1] != l ) || ( m_attenuation[2] != q ) )
    {
      m_attenuation[0] = c;
      m_attenuation[1] = l;
      m_attenuation[2] = q;
      notifyChange( this );
      notify( PID_Attenuation );
    }
  }
  ...
  inline  void  SpotLight::setFalloffExponent( float fe )
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_WRITE_LOCKED(this);
    if ( m_falloffExponent != fe )
    {
      m_falloffExponent = fe;
      notifyChange( this );
      notify( PID_FalloffExponent );
    }
  }
