...
namespace nvsg
{
  ...
  class MapObject : public Object
  {
    public:
      ...
      NVSG_API virtual void getSize( unsigned int & width
                                   , unsigned int & height ) const = 0;

      /*! \brief Interface to get an image at a specified raster position.
       *  \param x Horizontal raster coordinate for the image to get.
       *  \param y Vertical raster coordinate for the image to get.
       *  \return The TextureHost specifying the image at the specified raster position. */
      NVSG_API virtual const TextureHostSharedPtr & pickMap( int x, int y ) const = 0;

      REFLECTION_INFO_API( NVSG_API, MapObject );
    protected:
  ...
  inline bool pointInRect( int x, int y, int llx, int lly, unsigned int w, unsigned int h )
  {
    return( ( llx <= x ) && ( lly <= y ) && ( x <= (int)(llx+w) ) && ( y <= (int)(lly+h) ) );
  }

  inline bool pointInInterval( int x, int l, unsigned int s )
  {
