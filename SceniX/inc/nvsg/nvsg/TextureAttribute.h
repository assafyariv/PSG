...
#include "nvsg/TextureHost.h" // base class definition
...
namespace nvsg
{
  ...
  , TFM_MIN_BILINEAR = TFM_MIN_LINEAR                 //!< Deprecated, name and value provided for backwards compatibility.
  , TFM_MIN_LINEAR_MIPMAP_LINEAR                      //!< Corresponds to GL_LINEAR_MIPMAP_LINEAR
  , TFM_MIN_TRILINEAR = TFM_MIN_LINEAR_MIPMAP_LINEAR  //!< Deprecated, name and value provided for backwards compatibility.
  ...
  , TFM_MAG_BILINEAR = TFM_MAG_LINEAR   //!< Deprecated, name and value provided for backwards compatibility.
  ...
  class TextureAttributeItem : public OwnedObject<TextureAttribute>
  {
    friend class TextureAttribute;

    public:
      NVSG_API static TextureAttributeItemSharedPtr create();
  ...
  public:
    ...
    /*! \brief Returns the Texture assigned to the TextureAttributeItem.
      * \return 
      * The Texture currently assigned to the TextureAttributeItem.
      * \remarks
      * A Texture can be assigned to the TextureAttributeItem using setTexture.
      * \sa setTexture
      */
    NVSG_API const TextureSharedPtr & getTexture() const;

        /*! \brief Returns a cached value whether the assigned Texture is of type TextureHost.
      * \return 
      * The Texture currently assigned to the TextureAttributeItem is a sub-class of TextureHost
      */
    NVSG_API bool hasTextureHost() const;

    /*! \brief Assigns a Texture to the TextureAttributeItem.
     * \param texImg The Texture to assign.
     * \remarks A previously assigned Texture will be unassigned prior to assign the new
     * Texture specified by \a texImg. If the previously assigned Texture is not 
     * referenced by any other objects, it will be destroyed as it gets unassigned. */
    NVSG_API void setTexture( const TextureSharedPtr & texImg );
    
    /*! \brief Retrieves the file name associated with the attached TextureHost.
     * \return The file name associated with the attached TextureHost. 
     * If no TextureHost is assigned, an empty string will be returned. 
     ...
     * Accepted filter modes used for magnification are TFM_MAG_NEAREST, and TFM_MAG_LINEAR. 
     * The initial filter mode is TFM_MAG_LINEAR. 
     * \sa getMagFilterMode */
    ...
    /*! \brief Returns the anisotropic filtering value.
     * \returns A floating point number specifying the maximum anisotropic filtering value used.
     * \sa setMaxAnisotropy */ 
    NVSG_API float getMaxAnisotropy( void ) const;
    ...
    * Accepted filter modes used for minification are
    * TFM_MIN_NEAREST, TFM_MIN_LINEAR, TFM_MIN_LINEAR_MIPMAP_LINEAR, 
    * TFM_MIN_NEAREST_MIPMAP_NEAREST, TFM_MIN_NEAREST_MIPMAP_LINEAR,
    * and TFM_MIN_LINEAR_MIPMAP_NEAREST.
    * The four _MIPMAP_ filter modes require mipmaps available in the assigned Texture.
    * The initial filter mode is TFM_MIN_LINEAR.
    * \sa getMinFilterMode */
    NVSG_API void setMinFilterMode( TextureMinFilterMode mode );

    /*! \brief Set the maximum anisotropic filtering amount. 
     * \param anisotropy A floating point number specifying the maximum anisotropy filter to be used.
     * The default value is 1.0f for no anisotropic filtering. The input value must be greater or equal to 1.0f.
     * \sa getMaxAnisotropy */ 
    NVSG_API void setMaxAnisotropy( float anisotropy );

    /*! \brief Convenience function to check if a minification filter mode requires mipmaps.
     *  \param mode Specifies the minification filter mode to check. */
    NVSG_API bool isMipmapped( TextureMinFilterMode mode ) const; 
     ...
     * and TWCA_R. Accepted values for the wrap mode are TWM_REPEAT, TWM_CLAMP, TWM_MIRROR_REPEAT,
     * TWM_CLAMP_TO_EDGE, TWM_CLAMP_TO_BORDER, TWM_MIRROR_CLAMP, TWM_MIRROR_CLAMP_TO_EDGE, and
     * TWM_MIRROR_CLAMP_TO_BORDER.
    ...
    * \sa getMatrix, getTranslation. */
     ...
     * \n\n
     * A Texture that previously was assigned to the left-hand-sided 
     * TextureAttributeItem, will be unreferenced, causing the Texture
     * to get deleted if it is not reference anywhere else. 
     * \n\n
     * If the \a rhs TextureAttributeItem has a Texture assigned, the Texture
     * will not be deep-copied, but the same Texture will be assigned to the 
  ...
  protected:
    friend struct nvutil::Holder<TextureAttributeItem>;
      ...
      /*! \brief Default-constructs a TextureAttributeItem.
        ...
        * through TextureAttributeItem::create(), which returns a TextureAttributeItemSharedPtr
        * as follows:
        * \code
        *     TextureAttributeItemSharedPtr texAttribItem = TextureAttributeItem::create(); 
        * \endcode
        * A default constructed TextureAttributeItem has no TextureHost assigned to it.
        ...
        * SceniX provides the convenient helper types TextureAttributeItemReadLock and
        * TextureAttributeItemWriteLock to serve this purpose. Instantiated objects of these
        * types can be used similar as an ordinary pointer:
        * \code
        ...
        *       texAttribItem->setTexture(
        *         TextureHost::createFromFile("sunset.jpg", std::vector<std::string>()) );
        *       [...]
      ...
      /*! \brief Constructs a TextureAttributeItem as a copy from another TextureAttributeItem.
        ...
        * \n\n
        * If the \a rhs TextureAttributeItem has a Texture assigned, the Texture
        * will not be deep-copied, but the same Texture will be assigned to the new
      ...
      /*! \brief Destructs a TextureAttributeItem
        * \remarks A previously attached Texture will be unreferenced, causing it to get
  ...
  private:
      nvutil::SmartPtr<TextureAttributeItemData>  m_sharedData; 
      TextureSharedPtr                            m_texture; // texture image specification
      bool                                        m_textureIsHost; // avoid dynamic casts
  };

  inline bool TextureAttributeItem::hasTextureHost() const
  {
    return m_textureIsHost;
  }
  ...
  {
    public:
      NVSG_API static TextureAttributeSharedPtr create();
    ...
    public:
      ...
      NVSG_API void bindTextureAttributeItem( const TextureAttributeItemSharedPtr & item, unsigned int unit );
       ...
       * \code
       *    // assume hTexAttrib a valid TextureAttributeItemSharedPtr
       *
       *    // NOTE: 
       ...
       *    {
       *        TextureAttributeItemWriteLock theItem(texAttrib->getTextureAttributeItem(tu));
      ...
      NVSG_API unsigned int getNumberOfItems( void ) const;
       ...
       * \returns The TextureAttributeItem bound to the specified texture unit. */
      NVSG_API const TextureAttributeItemSharedPtr & getTextureAttributeItem( unsigned int unit ) const;
      ...
      * \sa TextureAttributeItem::isEquivalent */
      ...
      REFLECTION_INFO_API( NVSG_API, TextureAttribute );
    protected:
      friend struct nvutil::Holder<TextureAttribute>;

      /*! \brief Default-constructs a TextureAttribute.
       * \remarks A TextureAttribute will be instantiated and default constructed through
       * TextureAttribute::create(), which returns a TextureAttributeSharedPtr as follows:
       * \code
       *     TextureAttributeSharedPtr texAttrib = TextureAttribute::create(); 
       * \endcode
       ...
       * for either write or read-only operations on the object. SceniX provides 
       * the convenient helper types TextureAttributeWriteLock and TextureAttributeReadLock
       * to serve this purpose. Instantiated objects of these types can be used similar as
       * an ordinary pointer:
       * \code
       *     // generate a TextureAttributeItem to be bound to the TextureAttribute below
       *     TextureAttributeItemWeakPtr hTexAttribItem = TextureAttributeItem::create();
       *
       ...
       * \remarks All bound TextureAttributeItems will be unbound, and hence, unreferenced, causing 
    ...
    private:
      typedef std::map<unsigned int, TextureAttributeItemSharedPtr> TAIContainer;

    private:
      TAIContainer  m_bindings; // item bindings 
  };
  ...
  inline unsigned int TextureAttribute::getNextItem( unsigned int currItem ) const
  {
    ...
    {
      TAIContainer::const_iterator ci = m_bindings.upper_bound(currItem);
      return ci!=m_bindings.end() ? ci->first : INVALID_ITEM;
    }
    return INVALID_ITEM;
  }

  inline unsigned int TextureAttribute::getNumberOfItems( void ) const
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_READ_LOCKED(this);
    return( checked_cast<unsigned int>(m_bindings.size()) );
  }

  inline const TextureAttributeItemSharedPtr & TextureAttribute::getTextureAttributeItem( unsigned int unit ) const
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_READ_LOCKED(this);
    TAIContainer::const_iterator ci = m_bindings.find(unit);
    static TextureAttributeItemSharedPtr nullTextureAttributeItem;
    return ci!=m_bindings.end() ? ci->second : nullTextureAttributeItem;
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - TextureAttributeItem
  inline const TextureSharedPtr & TextureAttributeItem::getTexture() const
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_READ_LOCKED(this);
    return( m_texture );
  ...
  inline void TextureAttributeItem::setEnvColor( const nvmath::Vec4f & envColor )
  {
    ...
    if ( m_sharedData->m_envColor != envColor )
    {
      cloneIfShared(m_sharedData);
  ...
  inline float TextureAttributeItem::getMaxAnisotropy( void ) const
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_READ_LOCKED(this);
    return( m_sharedData->m_maxAnisotropy );
  }

  inline bool TextureAttributeItem::isMipmapped( TextureMinFilterMode mode ) const
  {
    NVSG_TRACE();
    NVSG_ASSERT(mode == TFM_MIN_NEAREST ||
                mode == TFM_MIN_LINEAR || 
                mode == TFM_MIN_LINEAR_MIPMAP_LINEAR || 
                mode == TFM_MIN_NEAREST_MIPMAP_NEAREST || 
                mode == TFM_MIN_NEAREST_MIPMAP_LINEAR || 
                mode == TFM_MIN_LINEAR_MIPMAP_NEAREST);
    return !( mode == TFM_MIN_LINEAR || mode == TFM_MIN_NEAREST );
  }
