...
namespace nvsg
{
  ...
  {
    public:
      NVSG_API static FaceAttributeSharedPtr create();
    ...
    public:
      ...
      /*! \brief Query if two-sided lighting is enabled.
      ...
      /*! \brief Query if face winding is set to counter clock wise.
       *  \return \c true, if counter clock wise face winding is activated, otherwise \c false. 
       *  \remarks The face winding flag specifies whether front faces are determined by counter 
       *  clock wise face winding or clock wise face winding. The default is true.
       *  \sa setFaceWindingCCW */
      NVSG_API bool isFaceWindingCCW() const;

      /*! \brief Set the face winding to counter clock wise or clock wise.
       *  \param ccw \c true to set face winding to counter clock wise, \c false to set face
       *  winding to clock wise. 
       *  \remarks The face winding flag specifies whether front faces are determined by counter 
       *  clock wise face winding or clock wise face winding. The default is true.
       *  Setting the face winding flag increases the nvutil::Incarnation.
       *  \sa isFaceWindingCCW() */
      NVSG_API void setFaceWindingCCW( bool ccw );
      ...
      REFLECTION_INFO_API( NVSG_API, FaceAttribute );
    protected:
      friend struct nvutil::Holder<FaceAttribute>;
  ...
  inline void FaceAttribute::setCullMode( bool frontCulling, bool backCulling )
  {
    ...
    if ( ( m_cullMode[0] != frontCulling ) || ( m_cullMode[1] != backCulling ) )
    {
  ...
  inline void FaceAttribute::setFaceMode( FaceMode frontMode, FaceMode backMode )
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_WRITE_LOCKED(this);
    if ( ( m_faceMode[0] != frontMode ) || ( m_faceMode[1] != backMode ) )
    {
  ...
  inline void FaceAttribute::setPolygonOffset( bool enabled )
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_READ_LOCKED(this);
    if ( m_polygonOffset.enabled != enabled )
    {
      m_polygonOffset.enabled = enabled;
      notifyChange( this );
    }
  }
  ...
  inline void FaceAttribute::setPolygonOffset( float factor, float units )
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_WRITE_LOCKED(this);
    if ( ( m_polygonOffset.factor != factor ) || ( m_polygonOffset.units != units ) )
    {
  ...
  inline void FaceAttribute::setTwoSidedLighting( bool enabled )
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_WRITE_LOCKED(this);
    if ( m_twoSidedLighting != enabled )
    {
  ...
  inline bool FaceAttribute::isFaceWindingCCW() const
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_READ_LOCKED(this);
    return( m_faceWindingCCW );
  }

  inline void FaceAttribute::setFaceWindingCCW( bool ccw )
  {
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_WRITE_LOCKED(this);
    if( m_faceWindingCCW != ccw )
    {
      m_faceWindingCCW = ccw;
      notifyChange( this );
    }
  }
