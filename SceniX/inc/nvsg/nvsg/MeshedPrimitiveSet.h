...
namespace nvsg
{
  /*! \brief IndexList specialization used with a MeshedPrimitiveSet.
   *  \par Namespace: nvsg
   *  \sa IndexList */
  class IndexMesh : public IndexList
  {
    public:
      IndexMesh( void );
   ...
   *  the width and height of the mesh and is an IndexList with width times height elements, and
   *  represents one mesh.
  ...
  class MeshedPrimitiveSet : public PrimitiveSet
  {
    public:
      ...
      NVSG_API unsigned int getNumberOfMeshes() const;
      ...
      NVSG_API void setMeshes( const IndexMesh * meshes, unsigned int numMeshes );
       ...
       *  IndexMesh located at \a meshes.
       *  \sa hasMeshes, getNumberOfMeshes, getMeshes */
      NVSG_API void setMeshes( unsigned int pos, const IndexMesh * meshes, unsigned int numMeshes );
      ...
      REFLECTION_INFO_API( NVSG_API, MeshedPrimitiveSet );
    protected:
      ...
      /*! \brief Calculates the bounding box of this MeshedPrimitiveSet.
       *  \return The axis-aligned bounding box of this MeshedPrimitiveSet.
       *  \remarks This function is called by the framework when re-calculation
       *  of the bounding box is required for this MeshedPrimitiveSet. */
    ...
    private:
      nvutil::SmartPtr<nvutil::RCVector<IndexMesh> > m_meshes;
  };
  ...
  inline unsigned int MeshedPrimitiveSet::getNumberOfMeshes()  const
  { 
    NVSG_TRACE();
    NVSG_ASSERT_OBJECT_READ_LOCKED(this);
    return( checked_cast<unsigned int>(m_meshes->size()) ); 
  }
