...
void CFileView::ClearList ()
{
	...
	HTREEITEM hRoot = m_wndFileView.InsertItem(_T("Models"), 0, 0);
	m_wndFileView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);
	m_list.clear();
}
void CFileView::AddFileLink (std::string name)
{
	HTREEITEM hRoot = m_wndFileView.GetRootItem();
	wchar_t wname[MAX_PATH];
	std::mbstowcs (wname,name.c_str(),MAX_PATH);
	::PathStripPath (wname);
	HTREEITEM hClass =  m_wndFileView.InsertItem(wname, 1, 1, hRoot);
	
	m_wndFileView.Expand(hRoot, TVE_EXPAND);

	m_list.push_back (pair<int,HTREEITEM> (0,hClass));
}
