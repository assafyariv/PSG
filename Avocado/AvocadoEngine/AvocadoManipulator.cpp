...
bool CylindricalCameraManipulator::updateFrame( float dt )
{
  ...
  if ( getViewState() && getRenderTarget() )
  {
    ...
    {
		// Assaf Temoporary removed this crap.
        // result = dolly();
    }
  }
  ...
  if ( viewState )
  {
    ...
    if ( isPtrTo<PerspectiveCamera>(theViewState->getCamera()) )
    {
      ...
      if (dy != 0)
      {
        ...
        theCamera->setFocusDistance(std::max(theCamera->getFocusDistance() - step, 0.1f));
...
bool CylindricalCameraManipulator::zoom()
{
  ...
  if ( viewState )
  {
    ...
    {
      CameraWriteLock theCamera(camera);
      int dy = getWheelTicks();//getCurrentY()-getLastY();
      if (dy != 0)
      {
        float targetDistance = theViewState->getTargetDistance();
...
bool CylindricalCameraManipulator::dollyZoom()
{
  ...
  if ( viewState )
  {
    ...
    if (camera)
    {
      ...
      int dy = getWheelTicks();//10;//getCurrentY()-getLastY();
      if (dy != 0)
      {
        float targetDistance = theViewState->getTargetDistance();
        float step =  m_speed * targetDistance * float(-dy);
   ...
   //     theCamera->setPosition(theCamera->getPosition()  - step * theCamera->getDirection());
	...
	//theCamera->setPosition(theCamera->getPosition() + step * theCamera->getDirection());
        ...
        theViewState->setTargetDistance(targetDistance - step);
		theViewState->setAutoClipPlanes (true);
        return true;
      }
    }
  }
...
bool CylindricalCameraManipulator::orbit()
{
  ...
  if ( viewState )
  {
	...
	//	theCamera->move(m_spinCenter, true); // world space
	//	m_spinCenter[1] = -m_spinCenter[1];
       theCamera->move(m_spinCenter, false);
		nvmath::Vec3f dir  = theCamera->getDirection();
	//	theCamera->setDirection(m_lookDir);

		float fd  = theCamera->getFocusDistance();
	//	theCamera->setFocusDistance(m_td);
		//
        theCamera->orbit( Vec3f(0.0f, 1.0f, 0.0f), theViewState->getTargetDistance(),  dx * PI_HALF, true ); // world relative
		  theCamera->orbit( Vec3f(1.0f, 0.0f, 0.0f), theViewState->getTargetDistance(),  -dy * PI_HALF, true ); //
       // theCamera->orbit( Vec3f(1.0f, 0.0f, 0.0f), theViewState->getTargetDistance(), -dy * PI_HALF, false  ); // camera relative
		theCamera->move(-1.0f*m_spinCenter, false); // world space
		//m_spinCenter[1] = -m_spinCenter[1];
        // redraw required
		//  theCamera->setDirection(dir);
		 
		//theCamera->setFocusDistance(fd);
        return true;
      }
    }
  }
...
bool CylindricalCameraManipulator::setPivot()
{
  ...
  {
    ViewStateReadLock theViewState(viewState);
	float vdist = theViewState->getTargetDistance();
    if ( isPtrTo<PerspectiveCamera>(theViewState->getCamera()) )
    {
      PerspectiveCameraSharedPtr camera( sharedPtr_cast<PerspectiveCamera>(theViewState->getCamera()) );
      PerspectiveCameraWriteLock theCamera(camera);
      ...
      if (vpW && vpH)
      {
		...
		float distance = 0.0f;
        if (picker->getNumberOfIntersections() > 0)
         distance = picker->getNearest().getDist();
		else
			distance=vdist;
...
bool CylindricalCameraManipulator::setSpincenter()
{
  ViewStateSharedPtr viewState = getViewState();
  if ( viewState )
  {
    ViewStateWriteLock theViewState(viewState);
	float vdist = theViewState->getTargetDistance();
    if ( isPtrTo<FrustumCamera>(theViewState->getCamera()) )
    {
      FrustumCameraSharedPtr camera( sharedPtr_cast<FrustumCamera>(theViewState->getCamera()) );
      FrustumCameraWriteLock theCamera(camera);
      ...
      if (vpW && vpH)
      {
        ...
        picker->apply(theViewState->getScene());
		float distance = 0.0f;
		nvmath::Vec3f bbcenter;
			bool centervalid = false;
        if (picker->getNumberOfIntersections() > 0)
		{
			
			if (picker->getNearest().getDrawable())
			{
				//DrawableReadLock d (picker->getIntersections()->getDrawable());
				
				//distance = nvmath::distance (bbcenter, theCamera->getPosition());
				centervalid = true;
distance = picker->getNearest().getDist();
const Path *pa = picker->getNearest().getPath ();
				...
				for (unsigned int i=0;i<pa->getLength();i++)
						{
							...
							if (parent)
							{
								...
								{
									 bbcenter =  GroupReadLock (parent)->getBoundingBox().getCenter();
									 distance = nvmath::distance (bbcenter, theCamera->getPosition());
									 foundElement = true;
									 break;
								}
				...
				if (!foundElement)
				{
					//get parent geo node rect..
					DrawableSharedPtr drwshrp = picker->getNearest().getDrawable();
					ObjectWeakPtr objppptr = /*DrawableWriteLock (drwshrp)->getOwner */*(DrawableReadLock (drwshrp)->ownersBegin());
					if (objppptr)
					{
						bbcenter = GeoNodeReadLock(objppptr)->getBoundingBox().getCenter();
						distance = nvmath::distance (bbcenter, theCamera->getPosition());
					}
				}
				//distance = picker->getNearest().getDist();
          ...
          if (0.0f < distance)
          {
            ...
            // Change the camera position instead of direction to make this pivot point setting roll free.
		//	theCamera->move(pivotNew - pivotOld, false); // world space
            m_td=std::max(0.1f, distance);
			m_lookDir = rayDir;
			theViewState->setTargetDistance(distance);
            // Also set the focus distance!
            // Do not focus nearer than lens' lower focus limit.
          //  distance = nvmath::distance( theCamera->getPosition(), pivotNew);
           theCamera->setFocusDistance(std::max(0.1f, distance));
			 m_spinCenter = pivotNew - pivotOld;
            // redraw required
            return true;
          }
      }
