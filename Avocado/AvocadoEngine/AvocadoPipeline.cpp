...
bool SceneRendererPipeline::SetAmbientOcclusionParams (nvsg::ViewStateSharedPtr vs, int width, int height)
{
	bool res = true;
	if (m_rendererPostprocAO)
	{
		...
		float farPlane = PerspectiveCameraReadLock(cam)->getFarDistance ();
		float nearPlane = PerspectiveCameraReadLock(cam)->getNearDistance ();
		float fov = PerspectiveCameraReadLock(cam)->getFieldOfView();
		float m_FocalLen[2] ;
		float m_InvResolution[2];
		m_FocalLen[0]      = 1.0f / tanf(fov * 0.5f) *  (float)height / (float)width;
        m_FocalLen[1]      = 1.0f / tanf(fov * 0.5f);
	...
	//	viewDir = traf * viewDir ;
		...
		//viewDir =  vv * w2v ;
		//viewDir.normalize ();
		float x = m_FocalLen[0];//viewDir[0];
		float y = m_FocalLen[1];//viewDir[1];
		float z = m_InvResolution[0];
		float w = m_InvResolution[1];
		...
		if (cgf)
		{
			...
			if (eff)
			{
				...
				CgFxParameter ptwo3w = CgFxEffectWriteLock (eff)->getTweakableByName ("gViewDirW");
				if (ptwo3w)
					CgFxEffectWriteLock (eff)->setFloatParameterValue (ptwo3w, &w);
				else res = false;
				CgFxParameter ptwo4 = CgFxEffectWriteLock (eff)->getTweakableByName ("gFar");
				if (ptwo4)
					CgFxEffectWriteLock (eff)->setFloatParameterValue (ptwo4, &farPlane);
				else res = false;
				CgFxParameter ptwo5 = CgFxEffectWriteLock (eff)->getTweakableByName ("gNear");
				if (ptwo5)
					CgFxEffectWriteLock (eff)->setFloatParameterValue (ptwo5, &nearPlane);
				else res = false;
                                 ...
                                 const SmartRenderTargetGL  &renderTarget)
{
 ...
 // CgFxEffectWriteLock(effect)->
  ...
  m_sketchFBO->setAttachment(nvgl::RenderTargetGLFBO::COLOR_ATTACHMENT1,
                                nvgl::TextureGLRectangle::create(GL_RGBA16, GL_BGRA, GL_UNSIGNED_BYTE));
  // Depth and Stencil are Renderbuffers.//GL_UNSIGNED_BYTE
  ...
  //TextureGLRectangleSharedPtr depthTex=  nvgl::TextureGLRectangle::create(GL_RGBA8, GL_BGRA, GL_UNSIGNED_BYTE);
 // m_sketchFBO->setAttachment(nvgl::RenderTargetGLFBO::DEPTH_STENCIL_ATTACHMENT,depthTex);
  // m_sketchFBO->setAttachment(nvgl::RenderTargetGLFBO::STENCIL_ATTACHMENT,depthTex);
  ...
  m_postprocFBO->setAttachment(nvgl::RenderTargetGLFBO::COLOR_ATTACHMENT0,
	//  nvgl::TextureGL2DMultisample::create (GL_RGBA8,8));
                                nvgl::TextureGLRectangle::create(GL_RGB8, GL_BGR, GL_UNSIGNED_BYTE));
  // Depth and Stencil are Renderbuffers.
...
//	  nvgl::TextureGL2DMultisample::create (GL_RGBA16,8));
  ...
  if (!m_sceneRendererHighlight)
  {
	...
	//	const RenderTargetGLFBO::SmartAttachment &attachment11_ppao = m_postprocFBO->getAttachment(RenderTargetGLFBO::COLOR_ATTACHMENT0);
	//	const RenderTargetGLFBO::SmartAttachmentTexture &texAtt11_ppao = smart_cast<RenderTargetGLFBO::AttachmentTexture>(attachment11_ppao);
	//	if (texAtt11_ppao)
	//	{
	//		const TextureGLSharedPtr &texGL11_ppao = texAtt11_ppao->getTextureGL();
	//		m_rendererPostprocAO->setSamplerTextureByName("gNormSampler", texGL11_ppao);
	//	}
#endif
		//m_imgProcEffect = hcg;
		// m_rendererHighlight uses the previously rendered texture rectangle as input for the CgFX shader.
...
void SceneRendererPipeline::SetGeoNodeTechnique(const GeoNodeSharedPtr geoNode, bool mrt/*, const StateSetSharedPtr & newSsh*/)
{
		GeoNodeWriteLock geoNodeLock(geoNode);
		...
		for ( GeoNode::StateSetConstIterator ssi = /*GeoNodeReadLock*/geoNodeLock->beginStateSets() ; ssi != ssiEnd ; ++ssi )
		{
			 StateSetWriteLock ssil (*ssi);
			  StatePass::AttributeIterator saciOne  = ssil->findAttribute( OC_CGFX );
			  if (saciOne == ssil->endAttributes())
				 continue;
		           CgFxSharedPtr tessCgFx  ( nvutil::sharedPtr_cast<CgFx>( *saciOne ) );
				   CgFxEffectSharedPtr cgfexffect = CgFxWriteLock(tessCgFx)->getEffect();
			if (!mrt)
			{
				  
				   CgFxTechnique tech = CgFxEffectWriteLock(cgfexffect)->getTechniqueByName ("Main");
				   if (tech)
					   CgFxEffectWriteLock(cgfexffect)->setCurrentTechnique (tech);
			}
			else
			{
				 CgFxTechnique tech = CgFxEffectWriteLock(cgfexffect)->getTechniqueByName ("MainMRT");
				   if (tech)
					   CgFxEffectWriteLock(cgfexffect)->setCurrentTechnique (tech);
			}
		}
...
void SceneRendererPipeline::SetShaderTechnique( const ViewStateSharedPtr &viewState, bool shaded)
{
	//static bool m_shaderstart = true;
	if (m_shaderstart )
	{
		...
		//	GeoNodeWriteLock node(dynamic_cast<GeoNodeWeakPtr>(*it));
			...
			//GeoNodeSharedPtr geonode = nvutil::sharedPtr_cast<GeoNode*>(*it);
			SetGeoNodeTechnique( geonode2,shaded/*new CommandReplaceStateSet( node, 0, newSsh )*/ );
		}
		if (m_cachedRoomTemp)
		{
			if (!shaded)
...
try
{
	...
	{
		 resetShader ();
	    SetShaderTechnique (viewState, false);
		  m_sceneRenderer->render(viewState, renderTarget , renderTarget->getStereoTarget());
	}
	else
	{
	   ...
	   if (m_postEnabledAO || m_postEnabledDOF)
	   {
	   		//doShader2(viewState);
		    resetShader ();
		   SetShaderTechnique (viewState, true);
		...
		//	m_sketchFBO->setSize((width * int(m_postprocFXAA)) / 2 , (height * int(m_postprocFXAA))/2);
		   m_sketchFBO->setSize((width * int(m_postprocFXAA))  , (height * int(m_postprocFXAA)));
			m_sketchFBO->setClearMask(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
			m_sceneRendererSketch->render (viewState,m_sketchFBO);
			//doShader2 (viewState,true);
		//	resetShader ();
  ...
  if (!(m_postEnabledAO || m_postEnabledDOF))
  {
	  resetShader ();
	   ...
	   if (m_postEnabledAO || m_postEnabledDOF)
	   {
		    SetAmbientOcclusionParams (viewState, width,height);
			SetAmbientOcclusionTechnique ("OcclusionMapOnly");
			
     	    m_postprocAOMAP->setSize((width * int(m_postprocFXAA)), (height * int(m_postprocFXAA)) );
			// whie bg for occ map
