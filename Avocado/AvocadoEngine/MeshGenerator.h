...
namespace nvutil
{
  ...
  nvsg::DrawableSharedPtr createCube(bool applyOptixCubeMapTextureCoords = false);
  ...
  // supported attributes: vertex, normal, texcoord0 (2D)
  nvsg::DrawableSharedPtr createSphere( unsigned int m , unsigned int n , float radius = 1.0f, bool cubemapTexcoords = false );
  ...
  // supported attributes: vertex, normal, texcoord0 (2D)
  nvsg::DrawableSharedPtr createCylinder( float r, float h, unsigned int hdivs, unsigned int thdivs, bool bOuter = true, float thstart = 0.0f, float thend = 2.0f*nvmath::PI , bool cone = false);
  
  //! Generate a cylinder with a radius of size \a radius, m edges in longitudinal and n edges in latitudinal direction, both m and n should be at least 3
  // supported attributes: vertex, normal, texcoord0 (2D)
  nvsg::DrawableSharedPtr createCylinderWithCap( float r, float h, unsigned int hdivs, unsigned int thdivs, bool bOuter = true, float thstart = 0.0f, float thend = 2.0f*nvmath::PI , bool cone = false);
  ...
  // supported attributes: vertex, normal, texcoord0 (2D)
  nvsg::DrawableSharedPtr createTorus( unsigned int m, unsigned int n, float innerRadius = 1.0f, float outerRadius = 0.5f , bool quarter = false);
  ...
  nvsg::DrawableSharedPtr createPlaneBorders( float x0, float y0, float width, float height, float wext = 1.0f, float hext = 1.0f);

  /* Simple line... */
   nvsg::DrawableSharedPtr createLine( float x0, float y0, float width, float height, float wext = 1.0f, float hext = 1.0f);

   nvsg::DrawableSharedPtr createPathLine( float x0, float y0, float z0,
   float x1, float y1, float z1);
  nvsg::DrawableSharedPtr createCallout( float x0, float y0, float z0,
   float x1, float y1, float z1);
  ...
  and the width \a width in pixels, and the height \a height in pixels of the rect. \a wext and \a hext can be set to specify how far
  the texture coordinates extend at the top right corner of the rect*/
  nvsg::DrawableSharedPtr createPlane( float x0, float y0, float width, float height, float wext = 1.0f, float hext = 1.0f, float depth = 0.0f);
