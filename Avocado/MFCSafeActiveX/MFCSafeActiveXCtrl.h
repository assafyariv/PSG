...
class CMFCSafeActiveXCtrl : public CActiveXDocControl
{
...
public:
	enum {
		...
		dispidHelloWorld = 1L,
		dispidSendAvocadoMsg = 4L,
		dispidLoadAvocadoFile = 5L
	};

	// Pointer to main dialog of the control
	CMainDialog * m_pMainDialog;
	CAvocadoView * m_pAvocadoView;
...
protected:
	...
	void SendAvocadoMsg (BSTR msg,BSTR params);
	void LoadAvocadoFile (BSTR fileUrl);
