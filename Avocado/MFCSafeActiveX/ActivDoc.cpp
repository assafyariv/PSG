        ...
        CFrameWnd* pOther)
{
    ...
    if (!m_pFrameWnd->Create(NULL, L"", WS_CHILD|WS_VISIBLE |  CS_OWNDC,
...
int CActiveXDocControl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	CView* pActiveView = ((CFrameWnd*) m_pFrameWnd)->GetActiveView();
	CDocument* pCurrentDoc = ((CFrameWnd*)m_pFrameWnd)->GetActiveDocument();
	...
	if (pos != NULL)
	{
		pCurrentDoc = GetDocTemplate()->GetNextDoc(pos);
	}
